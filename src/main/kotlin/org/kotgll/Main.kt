...
fun main(args: Array<String>) {
    ...
    val parallelMode by
    parser
        .option(ArgType.Choice<ParallelMode>(), fullName = "parallel", description = "Parallel mode")
        .required()
    val pathToInput by
    parser
    ...
    if (inputMode == InputMode.STRING) {
        ...
        if (grammarMode == GrammarMode.CFG) {
            ...
            if (sppfMode == SPPFMode.ON) {
                if (parallelMode == ParallelMode.ON) {
                    val result = org.kotgll.rsm.stringinput.withsppf.parallel.GLL(grammar, input).parse()
                    File(pathToOutput).printWriter().use { out -> out.println(result != null) }
                } else if (parallelMode == ParallelMode.OFF) {
                ...
                if (parallelMode == ParallelMode.ON) {
                    val result = org.kotgll.rsm.stringinput.withoutsppf.parallel.GLL(grammar, input).parse()
                    File(pathToOutput).printWriter().use { out -> out.println(result) }
                } else if (parallelMode == ParallelMode.OFF) {
        ...
        if (grammarMode == GrammarMode.CFG) {
            ...
            if (sppfMode == SPPFMode.ON) {
                ...
                if (parallelMode == ParallelMode.ON) {
                    val result = org.kotgll.rsm.graphinput.withsppf.parallel.GLL(grammar, graph).parse()
                    File(pathToOutput).printWriter().use { out ->
                        result.keys.forEach { tail ->
                            result[tail]!!.keys.forEach { head -> out.println("$tail $head") }
                        }
